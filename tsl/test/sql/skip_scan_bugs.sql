...
-- Make sure both compressed and uncompressed paths are included
-- as we can use SkipScan on compressed part but not on the uncompressed part
-- and we can use uncompressed index on (sales_day) because of predicate on sales_day.
:PREFIX SELECT count(DISTINCT style) FROM t_10421
